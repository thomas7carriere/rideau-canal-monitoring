...
1. Data is generated by the python Data Simulator application
2. The generated is data is sent to the IoT Hub's 3 devices, one for each location simulated
3. The IoT Hub device data is used as an input in the Stream Analytics Job
4. The raw data is output to the blob storage container. A query is also run on the data, with the aggregated data being sent to the Cosmos DB.
5. The dashboard application retrives the data from the Cosmos DB (every 30s) and updates the html dashboard representation
